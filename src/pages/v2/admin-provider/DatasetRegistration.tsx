...
import { Database, Plus, Layers, Pencil, Trash2 } from "lucide-react";
...
const DatasetRegistration = () => {
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      ...
      metadata: editingDataset?.metadata?.map((item: any) => ({
  ...
  return (
    <V2PageShell title="Dataset Registration" subtitle="Register, update, and remove provider datasets with backend-valid endpoint and metadata payloads." status="Live API">
      {isSuperAdmin && (
          ...
          >
            <DialogTrigger asChild>
              <Button
                ...
                title={
                  !domainId ? "Select a domain first"
                  : !providerParticipant ? "No provider participant resolved — register a KKKS in Admin Consumer → Admin Provider first"
                  : schemas.length === 0 ? "No schemas in this domain — create one in Master Data → Schemas first"
                  : !canManageDatasets ? "Missing permission datasets.manage / fulfilment.manage"
            ...
            <DialogContent className="sm:max-w-2xl">
              ...
              <div className="grid gap-4 py-4">
                <div className="grid gap-2 md:grid-cols-2">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select value={selectedSchemaId} onValueChange={(value) => setForm({ ...form, schema_id: value })}>
                      <SelectTrigger><SelectValue placeholder={loadingSchemas ? "Loading schemas..." : "Select schema"} /></SelectTrigger>
                      <SelectContent>
                        {schemas.map((schema) => (<SelectItem key={schema.id} value={schema.id}>{schema.version} - {schema.vocabulary_name || schema.id.slice(0, 8)}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  ...
                  <div className="grid gap-2">
                    <Label>Version *</Label>
                    <Input value={form.version} onChange={(event) => setForm({ ...form, version: event.target.value })} placeholder="0.0.1" />
                  </div>
                </div>

                <div className="grid gap-2">
                  <Label>Description</Label>
                  <Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Contract fulfilment dataset for GX-Space exchange." />
                </div>
        ...
        <CardContent>
          <DataTable headers={["Name", "Protocol", "Access", "Version", "Status", "Actions"]} isLoading={!domainId || loadingDatasets}>
            {datasets.length > 0 ? datasets.map((dataset) => (
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <Button
                      ...
                      variant="outline"
                      className="gap-2"
                      title="Backend health-check endpoint (/datasets/{id}/health) belum tersedia"
                      onClick={() => {
                        const ok = Math.random() > 0.25;
                        toast.info(`Stub health-check: ${ok ? "✓ endpoint reachable (simulated)" : "✗ unreachable (simulated)"}`, {
                          description: `${dataset.endpoint?.url || "no URL"} — backend belum tersedia`,
                          duration: 4000,
                        });
                      }}
                    >
              ...
              <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-muted-foreground">{domainId ? "No datasets registered" : "Select a domain"}</td></tr>
            )}
          </DataTable>
        </CardContent>
      </Card>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(nextOpen) => !nextOpen && setDeleteTarget(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deleteDataset.mutate({ domainId, id: deleteTarget.id }, { onSuccess: () => setDeleteTarget(null) })} className="bg-destructive">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
