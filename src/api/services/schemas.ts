...
export const schemasApi = {
  list: async (domainId: string): Promise<SchemaListResponse> => {
    ...
    const res = await ctsClient.get(`/data-catalog/${domainId}/schemas`);
  ...
  get: async (domainId: string, id: string): Promise<SchemaItem> => {
    const res = await ctsClient.get(`/data-catalog/${domainId}/schemas/${id}`);
  ...
  create: async (domainId: string, body: SchemaCreateRequest): Promise<SchemaItem> => {
    const res = await ctsClient.post(`/data-catalog/${domainId}/schemas`, body);
    const s = res.data as any;
    return {
      schema_id: s.schema_id ?? s.id,
      vocabulary_id: s.vocabulary_id,
  ...
  update: async (
    ...
    const res = await ctsClient.patch(`/data-catalog/${domainId}/schemas/${id}`, body);
    const s = res.data as any;
    return {
      schema_id: s.schema_id ?? s.id,
      vocabulary_id: s.vocabulary_id,
  ...
  remove: async (domainId: string, id: string): Promise<void> => {
    await ctsClient.delete(`/data-catalog/${domainId}/schemas/${id}`);
  },
};
